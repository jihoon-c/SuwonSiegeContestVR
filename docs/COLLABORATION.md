...
`57dd875`까지 커밋된 기존 템플릿 에셋(약 180개)은 **일반 Git 오브젝트로 이력에 남아 있다.**

* 앞으로 그 파일들을 **수정해서 커밋하면 그 시점부터 LFS로 전환**된다.
* 과거 이력까지 옮기려면 `git lfs migrate import --everything`이 필요하다.
* 이는 **이력 재작성 + force push**를 수반하며 **다른 개발자의 기존 clone을 전부 무효화**한다.
* 커밋이 2개뿐인 지금이 비용이 가장 낮은 시점이지만, **팀 합의 없이 실행하지 않는다.**
...
| 4 | 브랜치 정책 및 PR 규칙 확정 | §2 제안을 확정 규칙으로 승격 |
