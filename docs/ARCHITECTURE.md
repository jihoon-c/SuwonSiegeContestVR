...
| `USherpaVoiceRecognitionComponent` | `Core/Voice` | **실제 온디바이스 백엔드.** sherpa-onnx v1.13.6 + 한국어 Zipformer |
| `UMockVoiceRecognitionComponent` | `Core/Voice` | 마이크 없는 테스트용 대체 백엔드 |
| `UVoiceModelLibrary` | `Core/Voice` | 모델 경로 해석 · Android OBB 추출 |
...
**2026-08-27: 실제 백엔드 임포트 완료.** sherpa-onnx v1.13.6(Apache-2.0) C API와
`sherpa-onnx-streaming-zipformer-korean-2024-06-16` 모델을 연동했고, PC에서 실측 검증했다
(모델 로드 1.6초, 디코딩 RTF 0.06). 모델(141MB)은 커밋하지 않고
`Scripts/DownloadKoreanVoiceModel.py`로 내려받는다.
**엔진 `NNERuntimeORT` 플러그인은 onnxruntime.dll 충돌 때문에 비활성화했다.**
연동 상세와 Android 절차는 `docs/Core/specs/SHERPA_ONNX_INTEGRATION.md`,
후보 조사는 `docs/Core/specs/VOICE_RECOGNITION_BACKEND_SURVEY.md`,
시스템 사양은 `docs/Core/specs/INITIAL_CONSONANT_QUIZ.md`를 참조한다.
...
| 음성 인식 | Core | `Implemented (PC 검증 완료)` | `Source/SuwonSiegeContestVR/*/Core/Voice/` + `Source/ThirdParty/SherpaOnnx/` — Android 기기 검증 대기 |
...
| 5 | 음성 인식 **Android 기기 검증 미실시** | sherpa-onnx 연동과 PC 검증은 끝났으나, 개발 PC에 Android NDK가 없어 스탠드얼론 빌드를 만들지 못했다 | NDK 설치 후 `docs/Core/specs/SHERPA_ONNX_INTEGRATION.md` 6절 절차대로 패키징·기기 확인 |
