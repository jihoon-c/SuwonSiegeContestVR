...

{
    ...
    inline const TMap<FString, FString>& GetKoreanTranslations()
    {
        ...
        bLoaded = true;
        FString CsvContents;
        const FString CsvPath = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("TraceMotive/Config/Localization/TMLocalization.ko.csv"));
        ...
        {
            if (Row.Num() < 2 || FCString::Strcmp(Row[0], TEXT("Key")) == 0 || !Row[0][0] || !Row[1][0])
            {
                continue;
            }
