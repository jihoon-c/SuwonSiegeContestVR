...
private:
...

    {
...

        {
            ...
            return TMLoc::IsKoreanEditor()
                ...
                ? FText::FromString(FString::Printf(TEXT("검색 중: %d / %d 액터, 결과 %d개. 최대 %d개까지만 표시합니다."), ScanIndex, CandidateActors.Num(), Results.Num(), MaxResults))

                : FText::FromString(FString::Printf(TEXT("Scanning %d / %d actors, %d result(s). Results are capped at %d."), ScanIndex, CandidateActors.Num(), Results.Num(), MaxResults));
...

        {
            ...
            return TMLoc::IsKoreanEditor()
                ...
                ? FText::FromString(FString::Printf(TEXT("완료: 결과가 너무 많아 %d개에서 중단했습니다. 검색어를 더 구체적으로 입력하세요."), MaxResults))

                : FText::FromString(FString::Printf(TEXT("Done: stopped at %d results. Make the query more specific."), MaxResults));
...

    {
        ...
        while (Processed < TMPerf::OutlinerActorsPerTick() && ScanIndex < CandidateActors.Num() && Results.Num() < MaxResults && Budget.HasTime())
        ...
        if (Results.Num() >= MaxResults) bHitResultCap = true;

        bResultsDirty = true;

        RefreshResultsPanel();

        if (ScanIndex >= CandidateActors.Num() || Results.Num() >= MaxResults)
...

    {
...

        {
            ...
            if (Results.Num() >= MaxResults) return;
